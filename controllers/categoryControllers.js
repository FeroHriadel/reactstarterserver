...
exports.createCategory = async (req, res, next) => {
    try {
        ...
        const newCategory = new Category({title, slug: slugify(title.toLowerCase().trim()), description, image});
...
exports.updateCategory = async (req, res, next) => {
    try {
        let {title, description, _id, image} = req.body;
        ...
        const updatedCategory = await Category.findByIdAndUpdate(_id, {title, description, image}, {new: true});
...
exports.deleteCategoryv = async (req, res, next) => {
    try {
        const categoryId = req.params.categoryid;
        Category.findByIdAndRemove(categoryId).exec((err, deleted) => {
            if (err) return next(new ErrorResponse('Error. Category NOT deleted', 500));
